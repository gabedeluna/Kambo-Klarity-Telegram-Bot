...
async function fetchAvailability(startDateRange, endDateRange, sessionDurationMinutes) {
    try {
...
async function fetchMonthOverview(year, month, sessionDurationMinutes) {
    ...
    try {
        ...
        const allSlots = await fetchAvailability(startDateRange, endDateRange, sessionDurationMinutes);
...
function getSlotsForDate(dateString) {
    ...
    if (window.monthSlotData && window.monthSlotData[dateString]) {
        console.log(`Found ${window.monthSlotData[dateString].length} slots for ${dateString} in monthSlotData`);
    ...
    for (const [cacheKey, cachedData] of availabilityCache.entries()) {
        if (cacheKey.startsWith('overview-') && cachedData.overview && cachedData.overview.slotsByDate) {
