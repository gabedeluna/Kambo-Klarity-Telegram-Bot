...
| ID            | Task                                                                                                | Why / Acceptance Criteria                                                                                                                                                                                                                                                                                                                                                                                            |
| :------------ | :-------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [X]**PH6‑01** | **Implement Admin Designation (`bin/set_admin.js`)**                                                | Provide a script/mechanism to assign the 'admin' role to a specific user via Telegram ID. *Pass*: Script created, accepts TG ID, updates user role to 'admin' in DB (verified manually or via test DB state), uses core Prisma/Logger. Calls `setRoleSpecificCommands`.                                                                                                                                            |
| [X]**PH6‑02** | **Implement Role-Based Command Router & Refine Help Command**                                       | Implement `commandHandler.js` (or `commandRouter.js`) to handle role-based command routing. Update `commandRegistry.js`. Distinct `/help` for client/admin. *Pass*: Tests pass. Manual test shows distinct `/help` & role-based command access (admin can use admin commands, client cannot).                                                                                                                |
| [X]**PH6‑03** | **Consolidate Command Handling & Deprecate `commandRouter.js`**                                     | Modify `updateRouter.js` to correctly call `commandHandler.handleCommand`. Ensure `commandHandler.js` robustly implements role-based logic. Remove `src/middleware/commandRouter.js`. *Pass*: `updateRouter.js` calls `commandHandler.js`. `commandRouter.js` removed. Commands routed correctly by role. Manual tests pass. `architecture.md` updated.                                                            |
| [X]**PH6‑04** | **DB: Define `sessionType` Prisma Model & Migrate**                                                 | Define the `sessionType` model in `prisma/schema.prisma` (id, label, durationMinutes, description, price, active). Run `npx prisma migrate dev`. *Pass*: Migration successful, `sessionType` table exists in DB.                                                                                                                                                                                                 |
| [X]**PH6‑05** | **DB: Populate Initial Session Types**                                                              | Create a one-time script (e.g., `bin/populate_session_types.js`) or manually populate the `sessionType` table with initial data from `src/config/sessionTypes.json`. *Pass*: `sessionType` table contains the initial session types.                                                                                                                                                                             |
| [X]**PH6‑06** | **Core: Refactor `core/sessionTypes.js` for DB Access & Add CRUD Stubs**                              | Update `getAll()` to fetch active types from `prisma.sessionType`. Implement `getById(id)` from DB. Add (untested) stub functions for `createType`, `updateType`, `deleteTypeOrDeactivate` for future dashboard use. *Pass*: `getAll()`, `getById()` work correctly with DB. Unit tests for `getAll` and `getById` pass. Stubs exist.                                                                  |
| [X]**PH6‑07** | **Tool: Refactor `telegramNotifier.sendSessionTypeSelector` for DB Session Types**                    | Update tool in `src/tools/telegramNotifier.js` to use `core/sessionTypes.getAll({ active: true })` (from PH6-06). Ensure it still sends inline keyboard with `book_session:<sessionTypeId>` and stores `edit_msg_id` via `stateManager`. *Pass*: Tool sends selector with DB-driven types. Unit tests for the tool pass (mocking `core/sessionTypes` and `stateManager`).                               |
> _Completion Note (PH6-07, 2025-05-03): `sendSessionTypeSelector` in `telegramNotifier.js` updated to fetch session types from DB via `core/sessionTypes.js` and use `stateManager` to store `edit_msg_id`. Dependency injection for `stateManager` added to `createTelegramNotifier` and updated in `app.js`._
| [X]**PH6‑08** | **Command: Implement `/book` Client Command Handler**                                               | Create `src/commands/client/book.js`. Handler calls the refactored `telegramNotifier.sendSessionTypeSelector` (PH6-07). Register in `src/commands/registry.js`. *Pass*: `/book` command (client) triggers session type selector message. Manual test successful.                                                                                                                                                |
> _Completion Note (PH6-08, 2025-05-03): `/book` command handler implemented in `src/commands/client/book.js`. It uses the updated `sendSessionTypeSelector`. Command registered in `registry.js`. Initialization added to `app.js`._
| [ ]**PH6‑09** | **Handler: Enhance `callbackQueryHandler.js` for `/book` Flow**                                     | Ensure it correctly processes `book_session:<sessionTypeId>`, sets user state (`BOOKING`, `session_type`, `active_session_id`), clears `edit_msg_id`, invokes `bookingAgent.runBookingAgent` with initial prompt. Agent's first response edits original selector message. *Pass*: Callback works, state updated, agent invoked, message edited. Unit tests & manual test pass.                                    |
| [ ]**PH6‑10** | **Flow: Verify Agent/Graph Booking Conversation (Contextual Greeting, Slot Suggestion)**              | Manually test `/book` flow: contextual greeting (new/returning user), agent uses `getUserProfileData`, `getUserPastSessions`, `findFreeSlots` (stub), suggests slots. User can confirm a slot. *Pass*: Conversation flows logically, agent uses context and tools as expected (verified via logs/LangSmith).                                                                                                    |
| [ ]**PH6‑11** | **Flow: Implement Agent/Graph Booking Finalization (Store Data, Create GCal Stub Event, Send Waiver)** | On user slot confirmation in graph: nodes call `stateManager.storeBookingData` (updates `user.booking_slot`), `googleCalendar.createCalendarEvent` (stub, stores fake `googleEventId` in graph state), `telegramNotifier.sendWaiverLink` (stores new `edit_msg_id`). Agent gives concluding message. *Pass*: Graph nodes perform actions. DB/state updated. Waiver sent. Manual test, logs/LangSmith. |
| [ ]**PH6‑12** | **Notification: Implement Admin Notification for Tentative Booking**                                | After waiver link sent (PH6-11), a graph node or tool calls `telegramNotifier.sendAdminNotification` ("Client {Name} tentatively booked... Awaiting waiver."). *Pass*: Admin receives notification. Manual test.                                                                                                                                                                                              |
| [ ]**PH6‑13** | **Flow: Implement Pre-Waiver Cancellation Logic in Agent/Graph**                                    | If user cancels during booking graph: agent/graph detects intent. If `googleEventId` exists, node calls `googleCalendar.deleteCalendarEvent` (stub). Node calls `stateManager.resetUserState`. Agent confirms cancellation of *process*. Update agent prompt. *Pass*: Cancellation works, state reset, (stub) GCal event deleted. Manual test, logs/LangSmith.                                 |
| [ ]**PH6‑14** | **Command: Implement `/sessions` Admin Command (View Only)**                                        | Create `src/commands/admin/sessions.js`. Handler queries `prisma.sessions`, formats, and replies with list of upcoming/recent sessions. Register for 'admin' role. *Pass*: Admin can view sessions. Manual test.                                                                                                                                                                                                 |
| [ ]**PH6‑15** | **Command: Implement `/dashboard` Admin Command (WebApp Button)**                                     | Create `src/commands/admin/dashboard.js`. Handler replies with message + WebApp button to `config.FORM_URL/admin_dashboard`. Register for 'admin' role. *Pass*: Admin receives button. Manual test.                                                                                                                                                                                                     |
| [ ]**PH6‑16** | **Test Coverage & Documentation for Phase 6**                                                       | Ensure new/modified Phase 6 modules (`core/sessionTypes.js`, relevant tools, handlers, commands, graph nodes/edges) have unit tests where applicable, achieving target coverage. Add JSDoc for new public functions/modules. *Pass*: Coverage target met. Docs updated.                                                                                                                |
| [ ]**PH6‑17** | **Update `docs/architecture.md`**                                                                   | Update diagrams/descriptions to reflect any significant changes from Phase 6 (e.g., `SessionType` in DB, full booking flow activation). Update Phase 6 status. *Pass*: `architecture.md` is current.                                                                                                                                                                                               |
| [ ]**PH6‑18** | **Final Review & TASK.md Updates for Phase 6**                                                      | Tick all Phase 6 task boxes here when done. Ensure Discoveries/Insights are recorded. *Pass*: All Phase 6 tasks in `TASK.MD` checked. Discoveries documented.                                                                                                                                                                                                                               |

### Discovered During Work (Phase 6)

_(Add new subtasks here, e.g., `PH6‑D1`)_

### Insights & Decisions (Phase 6)
