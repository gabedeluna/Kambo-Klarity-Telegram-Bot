...
| [ ]**PH6‑06** | **Core: Refactor `core/sessionTypes.js` for DB Access & Add CRUD Stubs**                              | Update `getAll()` to fetch active types from `prisma.sessionType`. Implement `getById(id)` from DB. Add (untested) stub functions for `createType`, `updateType`, `deleteTypeOrDeactivate` for future dashboard use. *Pass*: `getAll()`, `getById()` work correctly with DB. Unit tests for `getAll` and `getById` pass. Stubs exist.                                                                  |
| [ ]**PH6‑07** | **Tool: Refactor `telegramNotifier.sendSessionTypeSelector` for DB Session Types**                    | Update tool in `src/tools/telegramNotifier.js` to use `core/sessionTypes.getAll({ active: true })` (from PH6-06). Ensure it still sends inline keyboard with `book_session:<sessionTypeId>` and stores `edit_msg_id` via `stateManager`. *Pass*: Tool sends selector with DB-driven types. Unit tests for the tool pass (mocking `core/sessionTypes` and `stateManager`).                               |
| [ ]**PH6‑08** | **Command: Implement `/book` Client Command Handler**                                               | Create `src/commands/client/book.js`. Handler calls the refactored `telegramNotifier.sendSessionTypeSelector` (PH6-07). Register in `src/commands/registry.js`. *Pass*: `/book` command (client) triggers session type selector message. Manual test successful.                                                                                                                                                |
...
```
| [X]**PH6‑02**    | **Role-Based Command Router & Refine Help Command**                                       | Implement `commandHandler.js` (or `commandRouter.js`) to handle role-based command routing. Update `commandRegistry.js`. Distinct `/help` for client/admin. *Pass*: Tests pass. Manual test shows distinct `/help` & role-based command access (admin can use admin commands, client cannot).                                                                                                                |
