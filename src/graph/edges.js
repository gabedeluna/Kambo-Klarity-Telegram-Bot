...
function routeAgentDecision(state) {
  ...
  if (toolCalls && toolCalls.length > 0) {
    ...
    if (!toolName) {
      logger.error(
        { toolCalls },
        "Agent tool_calls array is present but contains no valid tool name.",
      );
    ...
    logger.error(
      ...
      "Agent outcome unclear (no tool call or output). Routing to error.",
    );
...
function routeAfterSlotFinding(state) {
  logger.debug(
    { error: state.error, availableSlots: state.availableSlots },
    "Routing after slot finding...",
  );
  if (state.error) {
...
function routeAfterBookingStorage(state) {
  logger.debug(
    { error: state.error, bookingId: state.bookingId },
    "Routing after booking storage...",
  );
  if (state.error) {
  ...
  if (!state.bookingId) {
    logger.error(
      "Booking storage node finished, but no bookingId found in state.",
    );
...
function routeAfterGCalCreation(state) {
  logger.debug(
    { error: state.error, calendarEventId: state.calendarEventId },
    "Routing after GCal creation...",
  );
  if (state.error) {
  ...
  if (!state.calendarEventId) {
    logger.error(
      "Calendar event creation node finished, but no calendarEventId found in state.",
    );
    state.error =
      "Internal error: Calendar Event ID missing after creation step.";
    return "handleErrorNode";
  }
