...
describe('Booking Agent - Integration Tests', () => {
  let bookingAgent;
  let mockLLM, mockAgentExecutor, mockStateManager, mockTelegramNotifier, mockGoogleCalendar;
  let mockSessionMemory, mockConfig, mockLogger, mockUuid, mockBookingPromptTemplate;
  ...
  beforeEach(() => {
    ...
    mockStateManager = {
      getUserProfileData: sinon.stub().resolves({ success: true, data: { first_name: 'Tester', telegram_id: '123', active_session_id: 'existing-session' } }),
      getUserPastSessions: sinon.stub().resolves({ success: true, data: [{ session_id: 'past-session-1' }] }),
    ...
    mockGoogleCalendar = {
      findFreeSlots: sinon.stub().resolves({ success: true, data: ['Slot A', 'Slot B'] }),
      createCalendarEvent: sinon.stub().resolves({ success: true, data: { eventId: 'mock-event-id' } }),
    ...
    mockUuid = {
      v4: sinon.stub().returns('mock-session-id-123')
    };
    mockPrisma = { // Basic mock for Prisma Client
        users: { // Mock specific models/methods if needed by agent init directly
    ...
    mockBot = { // Basic mock for Telegraf bot instance
        telegram: { // Mock specific telegram methods if needed by agent init
          setMyCommands: sinon.stub().resolves(true),
        },
        // Add other bot methods like launch, use if checked in init
      };
    ...
    const mockStructuredToolInstance = { name: 'mockTool', description: 'mock tool desc' }; // Basic instance mock
    MockStructuredTool = sinon.stub().returns(mockStructuredToolInstance);

    // Mock message constructors from @langchain/core/messages
    MockSystemMessage = sinon.stub().returns({ _getType: () => 'system', content: 'mock system' });
    MockHumanMessage = sinon.stub().returns({ _getType: () => 'human', content: 'mock human' });
    MockAIMessage = sinon.stub().returns({ _getType: () => 'ai', content: 'mock ai' });
    MockToolMessage = sinon.stub().returns({ _getType: () => 'tool', content: 'mock tool', tool_call_id: 'mock-tool-id' });
    ...
    bookingAgent = proxyquire('../../agents/bookingAgent', {
      ...
      'langchain/agents': { 
        ...
        createOpenAIFunctionsAgent: sinon.stub().resolves(mockAgent) // Resolve to the mock agent object
      }, 
      '@langchain/core/prompts': {
        ChatPromptTemplate: { fromMessages: sinon.stub().returns(mockBookingPromptTemplate) }, // Mock prompt creation
        MessagesPlaceholder: sinon.stub().returns({ inputVariables: ['chat_history', 'agent_scratchpad'] }) // Mock placeholder including scratchpad
      },
      '@langchain/core/messages': {
       ...
       '@langchain/community/tools/google_calendar/index': { // Assuming this is where the tool wrappers are
      ...
      '../config/agentPrompts': { bookingAgentSystemPrompt: 'Test Prompt {user_name}', /* other prompts */ }, // Use real or test prompt
      ...
      'uuid': mockUuid,
       ...
       '@langchain/core/agents': { AgentFinish: sinon.stub(), AgentAction: sinon.stub() }
    });
  ...
  it('runBookingAgent should initialize, fetch data, manage session ID, and invoke the executor', async () => {
    // Arrange: Override specific mocks for this test
    mockStateManager.getUserProfileData.resolves({ success: true, data: { first_name: 'Tester', telegram_id: '123', active_session_id: null } }); // No active session
    mockStateManager.getUserPastSessions.resolves({ success: true, data: [{ session_id: 'prev_session'}] }); // Has past sessions
    ...
    // Act
    const result = await bookingAgent.runBookingAgent({ userInput: 'Hi', telegramId: '123' });
  ...
  it('runBookingAgent should handle intent to find slots (simplified check)', async () => {
    // Arrange - reset stubs for this test
    mockAgentExecutor.invoke.reset();
    mockAgentExecutor.invoke.resolves({ output: 'OK, I found these slots: Slot A, Slot B' });
    
    // Act
    const result = await bookingAgent.runBookingAgent({ userInput: 'Find me a slot', telegramId: '123' });
  ...
  it('runBookingAgent should handle cancellation intent (simplified check)', async () => {
    // Arrange
    mockAgentExecutor.invoke.resolves({ output: 'OK, booking cancelled.' });

    // Act
    const result = await bookingAgent.runBookingAgent({ userInput: 'cancel my booking', telegramId: '123' });

    // Assert
    expect(result.success).to.be.true;
    expect(result.output).to.equal('OK, booking cancelled.');
    expect(mockAgentExecutor.invoke).to.have.been.calledOnce;
  ...
  it('runBookingAgent should acknowledge first-time user based on past sessions', async () => {
    // Arrange
    mockStateManager.getUserProfileData.resolves({ success: true, data: { first_name: 'Newbie', telegram_id: '789', active_session_id: 'session-abc' } });
    mockStateManager.getUserPastSessions.resolves({ success: true, data: [] }); // No past sessions
    mockAgentExecutor.invoke.resolves({ output: 'Welcome, Newbie! Looks like your first time...' });

    // Act
    const result = await bookingAgent.runBookingAgent({ userInput: 'I want to book', telegramId: '789' });
   ...
   it('runBookingAgent should handle stateManager profile fetch failure', async () => {
    // Arrange
    mockStateManager.getUserProfileData.rejects(new Error('Database connection failed'));
