...
describe("Booking Agent - Integration Tests", () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      aiProvider: "openai", // Default to openai for tests
      NODE_ENV: "test",
    ...
    mockBot = {
      ...
      telegram: {
        // Mock specific telegram methods if needed by agent init
  ...
  it("runBookingAgent should handle intent to find slots (simplified check)", async () => {
    ...
    mockAgentExecutor.invoke.resolves({
      output: "OK, I found these slots: Slot A, Slot B",
    });
  ...
  it("runBookingAgent should acknowledge first-time user based on past sessions", async () => {
    ...
    mockAgentExecutor.invoke.resolves({
      output: "Welcome, Newbie! Looks like your first time...",
    });
