...
bot.on('message', async (ctx) => {
  try {
    ...
    console.log(`Received message from: ${firstName} ${lastName} (ID: ${userId})`);

    // Check if user exists and their role
    const existingUser = await prisma.users.findUnique({ where: { telegram_id: userId } });
    if (existingUser && existingUser.role === 'admin') {
      ...
      await ctx.reply('🟢 You were admin and have been demoted to client. Your commands are now default.');
      console.log('User was admin and has been demoted to client. Commands reset. Exiting...');
      process.exit(0);
    }
    ...
    const user = await prisma.users.upsert({
      ...
      create: { telegram_id: userId, first_name: firstName, last_name: lastName, role: 'admin' }
    });
    
    console.log('User registered as admin:', user);
    await ctx.reply(`✅ You (${firstName} ${lastName}) have been registered as an admin!`);
    ...
    if (error.message.includes('relation "users" does not exist') || error.message.includes('does not exist')) {
      console.error('Users table not found. Please create the users table manually in your database.');
    }
    await ctx.reply('❌ Error registering you as admin. Please check the logs.');
  }
});
