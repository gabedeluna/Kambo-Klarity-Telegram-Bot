...
const mockPrismaChildLogger = {
  // This is what appLogger.child will return
  info: jest.fn(),
  warn: jest.fn(),
  error: jest.fn(),
  fatal: jest.fn(),
...
describe("Core Prisma Client (prisma.js)", () => {
  ...
  it("should instantiate PrismaClient, create a child logger, and log instantiation", () => {
    ...
    expect(mockPrismaChildLogger.info).toHaveBeenCalledWith(
...
describe("Prisma Client Shutdown Handlers", () => {
  ...
  beforeEach(() => {
    ...
    _localPrismaInstance = require("../../src/core/prisma");
  });
  ...
  it("should call $disconnect on SIGINT", async () => {
    ...
    expect(mockPrismaClientInstance.$disconnect).toHaveBeenCalledTimes(1);
    expect(mockPrismaChildLogger.info).toHaveBeenCalledWith(
      "Received SIGINT. Disconnecting Prisma Client...",
    );
    expect(mockPrismaChildLogger.info).toHaveBeenCalledWith(
  ...
  it("should prevent multiple disconnects if already disconnecting", async () => {
    const beforeExitCallback = process.on.mock.calls.find(
    ...
    expect(mockPrismaChildLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining(
        "Prisma Client disconnection already in progress",
      ),
    );
  });
  ...
  it("should prevent disconnect if already disconnected", async () => {
    ...
    expect(mockPrismaChildLogger.debug).toHaveBeenCalledWith(
      expect.stringContaining("Prisma Client already disconnected"),
    );
  });
});
