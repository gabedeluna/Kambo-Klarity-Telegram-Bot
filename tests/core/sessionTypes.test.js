...
describe("Core Session Types (sessionTypes.js)", () => {
  beforeEach(() => {
    ...
    // Clear mock calls before each test
    Object.values(mockPrismaSessionTypeModel).forEach((mockFn) =>
      mockFn.mockClear(),
    );
  ...
  describe("getAll", () => {
    it("should call prisma.sessionType.findMany with correct parameters for active types by default", async () => {
      const mockSessionTypes = [
        { id: "1", label: "Test Session 1", active: true },
      ];
  ...
  describe("getById", () => {
    it("should call prisma.sessionType.findUnique with the provided id", async () => {
      ...
      mockPrismaSessionTypeModel.findUnique.mockResolvedValueOnce(
        mockSessionType,
      );
  ...
  describe("createType", () => {
    ...
    it("should call prisma.sessionType.create with data and return new type", async () => {
      ...
      expect(mockPrismaSessionTypeModel.create).toHaveBeenCalledWith({
        data: newTypeData,
      });
  ...
  describe("updateType", () => {
    ...
    it("should call prisma.sessionType.update with id and data, return updated type", async () => {
      ...
      expect(mockPrismaSessionTypeModel.update).toHaveBeenCalledWith({
        where: { id: typeId },
        data: updateData,
      });
    ...
    it("should return null and log warning for invalid ID", async () => {
      ...
      expect(result).toBeNull();
      expect(mockLogger.warn).toHaveBeenCalledWith(
        { idReceived: null, data: updateData },
  ...
  describe("deleteType", () => {
    ...
    it("should call prisma.sessionType.delete with id and return deleted type", async () => {
      ...
      expect(mockPrismaSessionTypeModel.delete).toHaveBeenCalledWith({
        where: { id: typeId },
      });
