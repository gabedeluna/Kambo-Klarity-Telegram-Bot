...
describe('GoogleCalendarEventsTool', () => {
  ...
  describe('createCalendarEvent', () => {
    ...
    it('should create event normally when buffer_time_minutes > 0', async () => {
      ...
      const result = await googleCalendarEventsTool.createCalendarEvent(mockEventDetails);
    ...
    it('should handle errors when fetching availability rules', async () => {
      // Mock database error
      mockPrisma.availabilityRule.findFirst.mockRejectedValue(new Error('Database error'));
    ...
    it('should handle Google Calendar API errors', async () => {
      ...
      mockCalendarEvents.insert.mockRejectedValue(new Error('Calendar API error'));
