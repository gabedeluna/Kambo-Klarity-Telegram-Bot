...
describe("Tools Telegram Notifier (telegramNotifier.js)", () => {
    ...
    it("should throw an error if dependencies are missing and log to console.error", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "FATAL: telegramNotifier initialization failed. Missing: bot, prisma, logger, config, sessionTypes, stateManager.",
        ),
      );
      ...
      expect(() =>
        createTelegramNotifier({ bot: {}, prisma: {}, logger: {}, config: {} }),
      ).toThrow(/Missing: config.formUrl, sessionTypes, stateManager/);
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "FATAL: telegramNotifier initialization failed. Missing: config.formUrl, sessionTypes, stateManager.",
        ),
      );
    });
  });
  ...
  describe("sendTextMessage", () => {
    it("should call bot.telegram.sendMessage with correct parameters", async () => {
      ...
      const result = await notifier.sendTextMessage({
        telegramId: testTelegramId,
        text,
      });

      expect(mockDependencies.bot.telegram.sendMessage).toHaveBeenCalledTimes(
        1,
      );
    ...
    it("should return success false if text is missing or empty", async () => {
      let result = await notifier.sendTextMessage({
        telegramId: testTelegramId,
      });
      expect(result.success).toBe(false);
      expect(result.error).toBe("Missing or invalid parameters");

      result = await notifier.sendTextMessage({
        telegramId: testTelegramId,
        text: "  ",
      });
      expect(result.success).toBe(false);
  ...
  describe("sendWaiverLink", () => {
    ...
    it("should send waiver link and store edit_msg_id successfully", async () => {
      ...
      const result = await notifier.sendWaiverLink({
        telegramId: testTelegramId,
        sessionType,
        messageText,
      });
      ...
      const sendMessageCall =
        mockDependencies.bot.telegram.sendMessage.mock.calls[0];
      ...
      expect(inlineKeyboard[0][0].web_app.url).toBe(
        `${mockConfig.formUrl}?telegramId=${testTelegramId}&sessionType=${encodeURIComponent(sessionType)}`,
      );
    ...
    it("should return success false on Telegram API error", async () => {
      mockDependencies.bot.telegram.sendMessage.mockRejectedValueOnce(
        new Error("TG API Fail"),
      );
      const result = await notifier.sendWaiverLink({
        telegramId: testTelegramId,
        sessionType,
    ...
    it("should return success true with warning if DB update for edit_msg_id fails", async () => {
      ...
      mockDependencies.prisma.users.update.mockRejectedValueOnce(
  ...
  describe("sendSessionTypeSelector", () => {
    ...
    it("should send selector with all types if user can book 3x3", async () => {
      mockDependencies.stateManager.getUserProfileData.mockResolvedValueOnce({
        success: true,
        data: { can_book_3x3: true },
      });
      mockDependencies.sessionTypes.getAll.mockResolvedValueOnce(
        mockActiveSessionTypes,
      );
      mockDependencies.bot.telegram.sendMessage.mockResolvedValueOnce({
        message_id: 111,
      });
      mockDependencies.stateManager.updateUserState.mockResolvedValueOnce({
        success: true,
      });

      const result = await notifier.sendSessionTypeSelector({
        telegramId: testTelegramId,
      });

      expect(
        mockDependencies.stateManager.getUserProfileData,
      ).toHaveBeenCalledWith({ telegramId: testTelegramId });
      expect(mockDependencies.sessionTypes.getAll).toHaveBeenCalledWith({
        active: true,
      });
      expect(mockDependencies.bot.telegram.sendMessage).toHaveBeenCalledTimes(
        1,
      ...
      expect(
        mockDependencies.stateManager.updateUserState,
      ).toHaveBeenCalledWith(testTelegramId, { edit_msg_id: 111 });
      expect(result.success).toBe(true);
    ...
    it("should send selector excluding 3x3 type if user cannot book 3x3", async () => {
      ...
      expect(
        keyboard.find((row) => row[0].text === "3 Hour Kambo (3x3)"),
      ).toBeUndefined();
    });
    ...
    it("should handle failure to fetch user profile", async () => {
      ...
      expect(mockDependencies.bot.telegram.sendMessage).toHaveBeenCalledWith(
        testTelegramId,
        expect.stringContaining(
          "Sorry, we encountered an issue retrieving your profile.",
        ),
      );
    });
    ...
    it("should handle no active session types found", async () => {
      ...
      expect(mockDependencies.bot.telegram.sendMessage).toHaveBeenCalledWith(
        testTelegramId,
        expect.stringContaining(
          "Sorry, there are currently no session types available for booking.",
        ),
      );
    });
  ...
  describe("sendAdminNotification", () => {
    ...
    const mockAdmins = [
      {
        telegram_id: BigInt(testAdminTelegramId),
        first_name: "Admin",
        last_name: "One",
      },
      {
        telegram_id: BigInt("112233445"),
        first_name: "Admin",
        last_name: "Two",
      },
    ];
    ...
    it("should send notifications to all admins successfully", async () => {
      ...
      mockDependencies.bot.telegram.sendMessage.mockResolvedValue({
        message_id: 1,
      }); // Simulate successful send for all
      ...
      expect(mockDependencies.bot.telegram.sendMessage).toHaveBeenCalledWith(
        String(mockAdmins[0].telegram_id),
        notificationText,
      );
      expect(mockDependencies.bot.telegram.sendMessage).toHaveBeenCalledWith(
        String(mockAdmins[1].telegram_id),
        notificationText,
      );
      expect(result.success).toBe(true);
    ...
    it("should return success false if DB error occurs fetching admins", async () => {
      mockDependencies.prisma.users.findMany.mockRejectedValueOnce(
        new Error("DB Read Fail"),
      );
      const result = await notifier.sendAdminNotification({
        text: notificationText,
      });
  ...
  describe("setRoleSpecificCommands", () => {
    ...
    let _originalCommandRegistry;

    beforeAll(() => {
      // Store original and replace
      _originalCommandRegistry = require("../../src/commands/registry");
    ...
    beforeEach(() => {
      ...
      const {
        createTelegramNotifier: createNotifierWithMockedRegistry,
      } = require("../../src/tools/telegramNotifier");
    ...
    it('should set client commands for "client" role', async () => {
      ...
      const result = await notifier.setRoleSpecificCommands({
        telegramId: testTelegramId,
        role: "client",
      });
    ...
    it("should return success false if telegramId or role is missing", async () => {
      ...
      res = await notifier.setRoleSpecificCommands({
        telegramId: testTelegramId,
      });
      expect(res.success).toBe(false);
      expect(res.error).toBe("Missing or invalid parameters");
      expect(
        mockDependencies.bot.telegram.setMyCommands,
      ).not.toHaveBeenCalled();
    });

    it("should return success false on Telegram API error", async () => {
      mockDependencies.bot.telegram.setMyCommands.mockRejectedValueOnce(
        new Error("TG API SetCommands Fail"),
      );
      const result = await notifier.setRoleSpecificCommands({
        telegramId: testTelegramId,
        role: "client",
