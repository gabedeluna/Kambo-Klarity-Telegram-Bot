...
jest.mock("date-fns-tz", () => ({
  ...
  fromZonedTime: jest.fn((date, tz) => mockActualDateFnsTz.fromZonedTime(date, tz)),
}));
...
describe("GoogleCalendarTool", () => {
  ...
  beforeEach(() => {
    ...
    dateFnsTz.toZonedTime.mockImplementation((date, tz) => mockActualDateFnsTz.toZonedTime(date, tz));
    dateFnsTz.fromZonedTime.mockImplementation((date, tz) => mockActualDateFnsTz.fromZonedTime(date, tz));
    
    originalEnv = { ...process.env };
    process.env.GOOGLE_APPLICATION_CREDENTIALS = "fake/path/to/creds.json";
    process.env.GOOGLE_CALENDAR_ID = "session_calendar_id@group.calendar.google.com";
  ...
  describe("constructor", () => {
    it("should instantiate successfully with all dependencies and env vars", () => {
      const tool = new GoogleCalendarTool({ logger: mockLogger, prisma: mockPrismaClient });
      ...
      expect(CurrentGoogle.calendar).toHaveBeenCalledWith({ version: "v3", auth: mockAuthInstance });
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("[GoogleCalendarTool] Live instance created."));
    });
    ...
    it("should use fallback pino logger if logger dependency is missing", () => {
      ...
      pinoInstanceIsolated.child.mockClear().mockImplementation(() => pinoInstanceIsolated);
      ...
      const consoleErrorSpyForTest = jest.spyOn(console, "error").mockImplementation(() => {});
      ...
      expect(pinoModuleIsolated.destination).toHaveBeenCalledWith(process.stdout);
    ...
    it("should log an error if GOOGLE_CALENDAR_ID is not set", () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith("CRITICAL: GOOGLE_CALENDAR_ID is not set in .env!");
    });
    ...
    it("should log an error if Google API client initialization fails", () => {
      ...
      FailingJWT.mockImplementationOnce(() => { throw new Error("Auth failed"); }); // Apply specific mock behavior
      ...
      const tool = new IsolatedFailingTool({ logger: currentMockLoggerForTest, prisma: mockPrismaClient });
  ...
  describe("getAvailabilityRule", () => {
    const mockDbRule = {
      id: "rule1", is_default: true, weekly_availability: { MON: [{ start: "10:00", end: "18:00" }] },
      practitioner_timezone: "America/Los_Angeles", max_advance_days: 30, min_notice_hours: 12,
      buffer_time_minutes: 15, max_bookings_per_day: 3, slot_increment_minutes: 15,
    };
    ...
    it("should parse weekly_availability if JSON string from Prisma", async () => {
      const ruleWithString = { ...mockDbRule, weekly_availability: JSON.stringify(mockDbRule.weekly_availability) };
      mockPrismaClient.availabilityRule.findFirst.mockResolvedValue(ruleWithString);
      const tool = new GoogleCalendarTool({ logger: mockLogger, prisma: mockPrismaClient });
      const rule = await tool.getAvailabilityRule();
    ...
    it("should handle error parsing weekly_availability JSON", async () => {
      const ruleWithInvalidJson = { ...mockDbRule, weekly_availability: "{invalid_json" };
      mockPrismaClient.availabilityRule.findFirst.mockResolvedValue(ruleWithInvalidJson);
      const tool = new GoogleCalendarTool({ logger: mockLogger, prisma: mockPrismaClient });
      const rule = await tool.getAvailabilityRule();
      expect(mockLogger.error).toHaveBeenCalledWith(expect.objectContaining({ err: expect.any(SyntaxError) }), "Failed to parse weekly_availability JSON from DB.");
    ...
    it("should use env placeholders if Prisma unavailable", async () => {
      ...
      const rule = await tool.getAvailabilityRule();
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("Prisma access not implemented"));
      expect(rule.weekly_availability).toEqual(JSON.parse(process.env.TEMP_WEEKLY_AVAILABILITY_JSON));
    });
    ...
    it("should use env placeholders if Prisma returns no rule", async () => {
      ...
      const tool = new GoogleCalendarTool({ logger: mockLogger, prisma: mockPrismaClient });
      const rule = await tool.getAvailabilityRule();
      expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining("no rule found"));
      expect(rule.weekly_availability).toEqual(JSON.parse(process.env.TEMP_WEEKLY_AVAILABILITY_JSON));
    });
    ...
    it("should throw and log error if Prisma call fails", async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith({ error: dbError }, "[getAvailabilityRule] Error fetching availability rule");
    });
  });
  ...
  describe("generateAvailableSlots", () => {
    ...
    beforeEach(() => {
        tool = new GoogleCalendarTool({ logger: mockLogger, prisma: mockPrismaClient });
    });

    it("should generate slots across multiple days (UTC context)", () => {
      const startDate = new Date("2024-01-01T00:00:00.000Z");
      const endDate = new Date("2024-01-02T23:59:59.000Z");
      const slots = tool.generateAvailableSlots(startDate, endDate, 60, [], baseAvailabilityRule);
  ...
  describe("findFreeSlots", () => {
    ...
    const mockAvailabilityRule = {
      weekly_availability: { MON: [{ start: "09:00", end: "17:00" }], TUE: [{ start: "10:00", end: "14:00" }] },
      practitioner_timezone: practitionerTz, max_advance_days: 60, min_notice_hours: 24,
      buffer_time_minutes: 15, max_bookings_per_day: 4, slot_increment_minutes: 30,
    };

    beforeEach(async () => {
      jest.setSystemTime(new Date("2024-07-15T04:00:00.000Z")); 
      toolInstance = new GoogleCalendarTool({ logger: mockLogger, prisma: mockPrismaClient });
      jest.spyOn(toolInstance, "getAvailabilityRule").mockResolvedValue(mockAvailabilityRule);
    ...
    it("should find available slots correctly with no GCal conflicts", async () => {
      const slots = await toolInstance.findFreeSlots({ 
          startDateRange: "2024-07-16T00:00:00Z", endDateRange: "2024-07-16T23:59:59Z", sessionDurationMinutes: 60 
      });
      expect(slots.length).toBe(7);
      expect(format(toZonedTime(parseISO(slots[0]), practitionerTz), "yyyy-MM-dd HH:mm")).toBe("2024-07-16 10:00");
    });

    it("should filter slots based on min_notice_hours", async () => {
      jest.setSystemTime(new Date("2024-07-15T14:00:00.000Z")); 
      const slots = await toolInstance.findFreeSlots({ 
          startDateRange: "2024-07-15T00:00:00Z", endDateRange: "2024-07-16T23:59:59Z", sessionDurationMinutes: 60 
      });
      expect(slots.length).toBe(7);
      expect(format(toZonedTime(parseISO(slots[0]), practitionerTz), "yyyy-MM-dd")).toBe("2024-07-16");
    });
    
    it("should filter by max_bookings_per_day", async () => {
      jest.spyOn(toolInstance, "getAvailabilityRule").mockResolvedValue({ ...mockAvailabilityRule, max_bookings_per_day: 1 });
      const eventStartNYT = set(toZonedTime(new Date("2024-07-16T00:00:00Z"), practitionerTz), { hours: 10 });
      const eventEndNYT = addMinutes(eventStartNYT, 60);
      mockGoogleCalendarEventsList
        .mockResolvedValueOnce({ data: { items: [{ start: { dateTime: fromZonedTime(eventStartNYT, practitionerTz).toISOString() }, end: { dateTime: fromZonedTime(eventEndNYT, practitionerTz).toISOString() } }] } })
        .mockResolvedValueOnce({ data: { items: [] } });
      const slots = await toolInstance.findFreeSlots({ startDateRange: "2024-07-16T00:00:00Z", endDateRange: "2024-07-16T23:59:59Z", sessionDurationMinutes: 60 });
      expect(slots.length).toBe(0);
    });
    
    it("should handle GCal API errors gracefully", async () => {
        mockGoogleCalendarEventsList.mockRejectedValueOnce(new Error("GCal API Error"));
        const slots = await toolInstance.findFreeSlots({ startDateRange: "2024-07-16T00:00:00Z", endDateRange: "2024-07-16T23:59:59Z", sessionDurationMinutes: 60 });
        expect(slots.length).toBe(0);
        expect(mockLogger.error).toHaveBeenCalledWith(expect.objectContaining({ message: "GCal API Error" }), expect.stringContaining("GCal Fetch Error"));
    });
    ...
    it("should return empty if rules or sessionCalendarId missing", async () => {
        ...
        let s = await toolInstance.findFreeSlots({ startDateRange: "2024-07-16T00:00:00Z", endDateRange: "2024-07-16T23:59:59Z", sessionDurationMinutes: 60 });
        expect(s.length).toBe(0);
        
        jest.spyOn(toolInstance, "getAvailabilityRule").mockResolvedValue(mockAvailabilityRule);
        toolInstance.sessionCalendarId = undefined;
        s = await toolInstance.findFreeSlots({ startDateRange: "2024-07-16T00:00:00Z", endDateRange: "2024-07-16T23:59:59Z", sessionDurationMinutes: 60 });
        expect(s.length).toBe(0);
    });
  });
});