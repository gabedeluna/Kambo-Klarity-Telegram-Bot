...
describe("Tools State Manager (stateManager.js)", () => {
  const _testUserId = "user123"; // Assuming this is a DB ID, not telegramId for some mocks
  ...
  describe("getUserProfileData", () => {
    it("should call prisma.users.findUnique with telegramId to get user profile", async () => {
      ...
      const result = await stateManager.getUserProfileData({
        telegramId: String(testTelegramId),
      });
    ...
    it("should return success false on database error", async () => {
      mockPrismaUserMethods.findUnique.mockRejectedValueOnce(
        new Error("DB Error"),
      );
      const result = await stateManager.getUserProfileData({
        telegramId: String(testTelegramId),
  ...
  describe("updateUserState", () => {
    ...
    const updatedUserMock = {
      telegram_id: testBigIntTelegramId,
      ...dataToUpdate,
    };
  ...
  describe("storeBookingData", () => {
    ...
    it("should return success false if bookingSlot is not a valid date string", async () => {
      ...
      expect(result.error).toBe(
        "Invalid input: bookingSlot must be a valid date or ISO string.",
      );
    });
  ...
  describe("setActiveSessionId", () => {
    ...
    it("should return success false if telegramId or sessionId is invalid", async () => {
      let result = await stateManager.setActiveSessionId({
        telegramId: null,
        sessionId,
      });
      expect(result.success).toBe(false);
      expect(result.error).toBe(
        "Invalid input: telegramId and sessionId are required.",
      );

      result = await stateManager.setActiveSessionId({
        telegramId: String(testTelegramId),
        sessionId: null,
      });
      expect(result.success).toBe(false);
