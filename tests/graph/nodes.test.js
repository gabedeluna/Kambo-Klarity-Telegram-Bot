...
describe("Graph Nodes", () => {
  ...
  describe("initializeNodes()", () => {
    it("should initialize successfully with all dependencies", () => {
      ...
      expect(mockLogger.info).to.have.been.calledWith(
        "[Graph Nodes] Initialized successfully.",
      );
    });
  ...
  describe("agentNode()", () => {
    it("should call bookingAgent and return successful outcome", async () => {
      ...
      const agentResult = {
        ...
        data: {
          ...
          details: "wants private session next week",
        },
      };
      ...
      expect(
        mockBookingAgent.runBookingAgent,
    ...
    it("should return error if bookingAgent call throws an exception", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Unexpected error during agent call/,
      );
    });
    ...
    it("should return error and log warning if userInput is missing", async () => {
      ...
      expect(result).to.deep.equal({
        agentOutcome: null,
        error: "User input missing for agent.",
      });
  ...
  describe("findSlotsNode()", () => {
    ...
    const slot2 = {
      start: "2025-05-11T14:00:00Z",
      end: "2025-05-11T15:30:00Z",
    };
    ...
    it("should call googleCalendar.findFreeSlots and return found slots", async () => {
      ...
      expect(
        mockGoogleCalendar.findFreeSlots.firstCall.args[0].durationMinutes,
      ).to.equal(90);
      expect(result).to.deep.equal({
        availableSlots: calendarResult.data,
    ...
    it("should return error if googleCalendar.findFreeSlots throws", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /\[Find Slots Node\] Unexpected error searching slots/,
      );
    });
  });
  ...
  describe("storeBookingNode()", () => {
    ...
    it("should call stateManager.storeBookingData successfully", async () => {
      ...
      expect(
        mockStateManager.storeBookingData,
      ).to.have.been.calledOnceWithExactly({
        telegramId: initialState.telegramId,
      ...
      expect(result).to.deep.equal({
        lastToolResponse: "Booking data stored.",
      });
      expect(result.error).to.be.undefined;
      expect(mockLogger.info).to.have.been.calledWithMatch(
        /Booking data stored/,
    ...
    it("should return error if stateManager.storeBookingData throws", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Unexpected error storing booking/,
      );
    });
    ...
    it("should return error if confirmedSlot is missing or invalid", async () => {
      ...
      for (const initialState of invalidStates) {
        ...
        expect(mockLogger.error).to.have.been.calledWithMatch(
          /Invalid or missing confirmedSlot/,
        );
      }
    });
  });
  ...
  describe("createCalendarEventNode()", () => {
    ...
    it("should call googleCalendar.createCalendarEvent successfully", async () => {
      ...
      const expectedEventDetails =
        mockGoogleCalendar.createCalendarEvent.firstCall.args[0];
      expect(expectedEventDetails.start).to.equal(confirmedSlot.start);
      expect(expectedEventDetails.end).to.equal(confirmedSlot.end);
      expect(expectedEventDetails.summary).to.contain(
        "Kambo Session (private) with Test User",
      );
      expect(result).to.deep.equal({
      ...
      expect(result.error).to.be.undefined;
      expect(mockLogger.info).to.have.been.calledWithMatch(
        /Calendar event created \(ID: evt12345\)/,
      );
    });
    ...
    it("should return error if googleCalendar.createCalendarEvent throws", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Unexpected error creating GCal event/,
      );
    });
  ...
  describe("sendWaiverNode()", () => {
    it("should call telegramNotifier.sendWaiverLink successfully", async () => {
      ...
      expect(
        mockTelegramNotifier.sendWaiverLink,
      ).to.have.been.calledOnceWithExactly({
        telegramId: initialState.telegramId,
        sessionType: initialState.sessionType,
      });
    ...
    it("should return error if telegramNotifier.sendWaiverLink throws", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Unexpected error sending waiver/,
      );
    });
  });
  ...
  describe("resetStateNode()", () => {
    it("should call stateManager.resetUserState successfully", async () => {
      ...
      expect(
        mockStateManager.resetUserState,
      ).to.have.been.calledOnceWithExactly({
        telegramId: initialState.telegramId,
      });
    ...
    it("should return error if stateManager.resetUserState throws", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Unexpected error resetting state/,
      );
    });
  });
  ...
  describe("handleErrorNode()", () => {
    it("should log the error and attempt to notify the user", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Entering for user: userErr. Error: Something went wrong/,
      );
      expect(mockTelegramNotifier.sendTextMessage).to.have.been.calledOnce;
      const notificationArgs =
        mockTelegramNotifier.sendTextMessage.firstCall.args[0];
      ...
      expect(mockLogger.info).to.have.been.calledWithMatch(
        /Notified user userErr about the error/,
      );
    ...
    it("should log the error even if notification fails", async () => {
      ...
      expect(mockLogger.error).to.have.been.calledWithMatch(
        /Entering for user: userErrNotifyFail. Error: Another issue/,
      );
      expect(mockTelegramNotifier.sendTextMessage).to.have.been.calledOnce;
      expect(mockLogger.error).to.have.been.calledWithMatch(
    ...
    it("should handle cases where error is an object or null/undefined", async () => {
      ...
      for (const initialState of states) {
        ...
        expect(
          mockTelegramNotifier.sendTextMessage.firstCall.args[0].text,
  ...
  // --- sendTextMessageNode tests ---
  describe("sendTextMessageNode()", () => {
    it("should send message successfully when text is available", async () => {
      const agentOutput = "Your session has been confirmed!";
      const initialState = getBaseState({
      ...
      expect(mockTelegramNotifier.sendTextMessage).to.have.been.calledWith({
        telegramId: initialState.telegramId,
        text: agentOutput,
      });
    ...
    it("should return error when no text is available", async () => {
      const initialState = getBaseState({
        agentOutcome: null, // No output text available
      });
    ...
    it("should handle Telegram API failure", async () => {
      const agentOutput = "Test message";
      const initialState = getBaseState({
        agentOutcome: { output: agentOutput },
      });
  ...
  // --- deleteCalendarEventNode tests ---
  describe("deleteCalendarEventNode()", () => {
    it("should delete calendar event successfully", async () => {
      const initialState = getBaseState({
        googleEventId: "event123",
      });
      mockGoogleCalendar.deleteCalendarEvent.resolves({ success: true });

      const result = await deleteCalendarEventNode(initialState);

      expect(mockGoogleCalendar.deleteCalendarEvent).to.have.been.calledWith({
        eventId: "event123",
      });
      expect(result).to.deep.equal({
    ...
    it("should return error when no event ID is available", async () => {
      const initialState = getBaseState({
        googleEventId: null, // No event ID available
      });
    ...
    it("should handle Google Calendar API failure", async () => {
      const initialState = getBaseState({
        googleEventId: "event123",
      });
