...
function spawnProcess(command, args, name, color) {
  console.log(
    `${color}🚀 [STARTUP/${name}] Starting ${name}...${colors.reset}`,
  );
  ...
  childProcess.on("exit", (code) => {
    if (code === 0) {
      console.log(
        `${colors.success}✅ [STARTUP/${name}] Process exited successfully${colors.reset}`,
      );
    } else {
      console.log(
        `${colors.error}❌ [STARTUP/${name}] Process exited with code ${code}${colors.reset}`,
      );
    }
  });
  ...
  childProcess.on("error", (err) => {
    console.log(
      `${colors.error}❌ [STARTUP/${name}] Failed to start: ${err.message}${colors.reset}`,
    );
  });
...
console.log(
  `🔄 [STARTUP] Starting Kambo Klarity system at ${new Date().toISOString()}`,
);
...
console.log(
  "✅ [STARTUP] All processes started. Press Ctrl+C to stop everything.",
);
