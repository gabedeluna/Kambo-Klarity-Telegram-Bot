  ...
  async trigger(ctx) {
    ...
    if (isSessionTypeCallback) {
      try {
        ...
        console.error(
          `❌ [${wf}/sessionTypeCallback] Error handling session type callback:`,
          error,
        );
        await ctx.reply(
          "Sorry, there was an error starting your booking. Please try again.",
        );
        return;
      }
    }
    ...
    if (!isBookCommand && !isSessionTypeCallback && !isInBookingState) {
      console.log(
        `⏭ [${wf}/entry] Skipped (not a booking command, session callback, or user in BOOKING state)`,
      );
      return;
    }
    ...
    if (
      (isBookCommand || isInBookingState) &&
    ...
    console.log(
      `🔄 [${wf}/entry] Handling booking workflow for user: ${ctx.from.id}`,
    );
    ...
    if (isBookCommand) {
      try {
        console.log(
          `🔄 [${wf}/showSessionOptions] Showing session options to user: ${ctx.from.id}`,
        );
...
👇 Tap one of the buttons below to choose\\.`;
        ...
        console.log(
          `✅ [${wf}/showSessionOptions] Session options displayed successfully`,
        );
      } catch (error) {
        console.error(
          `❌ [${wf}/showSessionOptions] Error showing session options:`,
          error,
        );
        await ctx.reply(
          "Sorry, I couldn't show the session options. Please try again later.",
        );
      }
      return;
    }
    ...
    if (isSessionTypeCallback) {
      try {
        ...
        console.log(
          `🔄 [${wf}/handleSessionSelection] User selected session type: ${sessionType}`,
        );
...
When would you like to book your ${sessionType} session?`;
        ...
        try {
          ...
          console.log(
            `✅ [${wf}/handleSessionSelection] Updated user state to BOOKING and saved session type: ${sessionType}`,
          );
        } catch (dbError) {
          console.error(
            `❌ [${wf}/handleSessionSelection] Database error while updating user state:`,
            dbError,
          );
        }

        console.log(
          `✅ [${wf}/handleSessionSelection] Scheduling instructions sent for session type: ${sessionType}`,
        );
      } catch (error) {
        console.error(
          `❌ [${wf}/handleSessionSelection] Error handling session selection:`,
          error,
        );
      }
      return;
    ...
    if (isInBookingState && ctx.message?.text && !isCancelRequest) {
      try {
        ...
        console.log(
          `🔄 [${wf}/handleDateTimeInput] Received date/time input: ${userInput}`,
        );
        ...
        try {
          ...
          console.error(
            `❌ [${wf}/handleDateTimeInput] Error parsing conversation history:`,
            parseError,
          );
        ...
        console.log(
          `🔄 [${wf}/handleDateTimeInput] Sending to AI agent with provider: ${provider}`,
        );
        ...
        console.log(
          `✅ [${wf}/handleDateTimeInput] Processed user input with AI agent`,
        );
      } catch (error) {
        console.error(
          `❌ [${wf}/handleDateTimeInput] Error processing with AI agent:`,
          error,
        );
        await ctx.reply(
...
async function handleAIResponse(ctx, aiResponse, wf) {
  const { responseType, message, suggestedSlots, confirmedSlot, toolCalls } =
    aiResponse;

  console.log(
    `🔄 [${wf}/handleAIResponse] Processing AI response type: ${responseType}`,
  );
...
async function executeToolCall(ctx, toolCall, wf) {
  ...
  try {
    ...
    console.error(
      `❌ [${wf}/executeToolCall] Error executing tool ${tool}:`,
      error,
    );
  }
}
...
async function handleCancelBooking(ctx, wf) {
  try {
    console.log(
      `🔄 [${wf}/handleCancelBooking] User requested to cancel booking: ${ctx.from.id}`,
    );
    ...
    await ctx.reply(
      "Booking process canceled. You can start over anytime with the /book command.",
    );
    console.log(
      `✅ [${wf}/handleCancelBooking] Booking canceled, user state reset to NONE`,
    );
  } catch (error) {
    console.error(
      `❌ [${wf}/handleCancelBooking] Error canceling booking:`,
      error,
    );
    await ctx.reply(
      "There was an issue canceling your booking. Please try again or contact support.",
    );
  }
}
