...
module.exports = {
  ...
  async processWaiverSubmission(formData) {
    ...
    try {
      ...
      console.log(
        `🔄 [${wf}/process/user-lookup] Looking up user with telegram_id: ${telegramId}`,
      );
      ...
      if (!user) {
        console.error(
          `❌ [${wf}/process/user-lookup] User not found: ${telegramId}`,
        );
        return { success: false, message: "User not found" };
      }
      console.log(
        `✅ [${wf}/process/user-lookup] Found user: ${user.first_name} ${user.last_name}`,
      );
      ...
      console.log(
        `🔄 [${wf}/process/session-creation] Creating session record`,
      );
      ...
      console.log(
        `✅ [${wf}/process/session-creation] Created session with ID: ${session.id}`,
      );
      ...
      console.log(
        `🔄 [${wf}/process/user-update] Updating user record for: ${telegramId}`,
      );

      // Log changes for tracking
      if (firstName !== user.first_name || lastName !== user.last_name) {
        console.log(
          `🔄 [${wf}/process/user-update] Updating name from "${user.first_name} ${user.last_name}" to "${firstName} ${lastName}"`,
        );
      }
      if (email !== user.email) {
        console.log(
          `🔄 [${wf}/process/user-update] Updating email from "${user.email}" to "${email}"`,
        );
      }
      if (phone !== user.phone_number) {
        console.log(
          `🔄 [${wf}/process/user-update] Updating phone from "${user.phone_number}" to "${phone}"`,
        );
      }
      ...
      console.log(
        `✅ [${wf}/process/user-update] Updated user profile with all form data`,
      );
      console.log(
        `✅ [${wf}/process] Waiver submission processed successfully`,
      );
  ...
  async handleWaiverSubmission(ctx, sessionId, workflowName) {
    ...
    try {
      ...
      console.log(
        `🔄 [${wf}/handle/session-lookup] Looking up session with ID: ${sessionId}`,
      );
      ...
      console.log(
        `🔄 [${wf}/handle/session-update] Updating session status to CONFIRMED`,
      );
      ...
      await ctx.editMessageText(
        "❌ There was an error processing your booking. Please contact support.",
        { parse_mode: "HTML" },
      );
    }
  },
