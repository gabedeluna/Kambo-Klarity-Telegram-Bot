...
module.exports = {
  ...
  async trigger(ctx) {
    ...
    if (ctx.state.user?.role !== "admin") {
      console.log(
        `⏭ [${wf}/entry] Skipped (role is not admin or user data missing)`,
      );
      return;
    }
    console.log(
      `🔄 [${wf}/entry] Handling admin command from user ${ctx.state.user.client_id}`,
    );
    ...
    await ctx.reply(
      `Acknowledged, Admin ${ctx.state.user.first_name || ""}! (Admin Workflow)`,
    );
