...
const resetState = async (telegramId) => {
  try {
    console.log(`🔄 [bookingTools/resetState] Resetting state for user: ${telegramId}`);
    ...
    console.log(`✅ [bookingTools/resetState] Successfully reset state for user: ${telegramId}`);
    return { success: true };
  } catch (error) {
...
const sendForm = async (telegramId, sessionType, telegram) => {
  try {
    console.log(`🔄 [bookingTools/sendForm] Sending booking form for ${sessionType} to user: ${telegramId}`);
    ...
    console.log(`🔄 [bookingTools/sendForm/message] Sending booking message and tracking ID`);
    ...
    const sentMessage = await telegram.sendMessage(
      ...
      message,
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "Book Now", web_app: { url: formUrl } }]
          ]
        }
      }
    );
    ...
    if (sentMessage && sentMessage.message_id) {
      console.log(`🔄 [bookingTools/sendForm/tracking] Storing message ID: ${sentMessage.message_id} for user: ${telegramId}`);
      ...
      console.log(`✅ [bookingTools/sendForm/tracking] Successfully stored message ID: ${sentMessage.message_id} for user: ${telegramId}`);
    } else {
      console.warn(`⚠️ [bookingTools/sendForm/tracking] Could not get message ID from sent message`);
    }
    
    console.log(`✅ [bookingTools/sendForm] Successfully sent booking form to user: ${telegramId}`);
    return { success: true };
  } catch (error) {
    console.error(`❌ [bookingTools/sendForm] Error sending booking form:`, error);
    return { 
      success: false,
      error: error.message
    };
...
const storeConfirmedSlot = async (telegramId, confirmedSlot) => {
  try {
    console.log(`🔄 [bookingTools/storeConfirmedSlot] Storing confirmed slot for user: ${telegramId}`);
    ...
    console.log(`✅ [bookingTools/storeConfirmedSlot] Successfully stored confirmed slot for user: ${telegramId}`);
    return { success: true };
  } catch (error) {
    console.error(`❌ [bookingTools/storeConfirmedSlot] Error storing confirmed slot:`, error);
    return { 
      success: false,
      error: error.message
    };
