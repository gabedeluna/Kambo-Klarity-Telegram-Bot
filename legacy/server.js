...
app.get('/api/user-data', async (req, res) => {
  ...
  console.log(`🔄 [server/userData] Fetching user data for telegramId: ${telegramId}`);
  ...
  try {
    ...
    if (formattedDateTime && formattedDateTime !== 'Not scheduled') {
      try {
        ...
        const datePart = format(parseISO(formattedDateTime), "EEEE, MMMM d, yyyy");
    ...
    console.log(`🔄 [server/userData/assembly] Assembling user data with emergency contact info`);
    const userData = {
    ...
    if (user.em_first_name || user.em_last_name || user.em_phone_number) {
      console.log(`✅ [server/userData/assembly] Found emergency contact info to auto-populate`);
    }

    
    console.log(`🔄 [server/userData] Appointment data being sent: "${formattedDateTime}"`);
    console.log(`🔄 [server/userData] Phone field being sent: "${user.phone_number || 'none'}"`);
...
app.post('/api/submit-waiver', async (req, res) => {
  ...
  try {
    ...
    if (result.success && result.sessionId) {
      console.log(`🔄 [server/submitWaiver/notification] Notifying bot about session ${result.sessionId}`);
      ...
      console.log(`🔄 [server/submitWaiver/user-lookup] Looking up user data for telegramId: ${telegramId}`);
      ...
      try {
        ...
        if (user && user.edit_msg_id) {
          console.log(`🔄 [server/submitWaiver/tracking] Found message ID: ${user.edit_msg_id} for user: ${telegramId}`);
          ...
          console.log(`🔄 [server/submitWaiver/tracking/cleanup] Clearing message ID for user: ${telegramId}`);
          ...
          try {
            ...
            console.log(`✅ [server/submitWaiver/tracking/cleanup] Successfully cleared message ID`);
          } catch (cleanupError) {
            console.error(`❌ [server/submitWaiver/tracking/cleanup] Error clearing message ID:`, cleanupError);
          }
        } else {
          console.warn(`⚠️ [server/submitWaiver/tracking] No message ID found for user: ${telegramId}`);
        }
        
        // Send webhook to bot server
        const botServerUrl = process.env.BOT_SERVER_URL || 'http://localhost:3000';
        ...
        if (notifyResponse.ok) {
          console.log(`✅ [server/submitWaiver/notification] Bot notification successful`);
        } else {
          console.error(`❌ [server/submitWaiver/notification] Bot notification failed:`, 
                      await notifyResponse.text());
        }
      } catch (notifyError) {
        // Don't fail the submission if notification fails
        console.error(`❌ [server/submitWaiver/notification] Error notifying bot:`, notifyError);
      }
    }
    ...
    console.error(`❌ [server/submitWaiver] Error forwarding to workflow:`, error);
